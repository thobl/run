...
def add(name,
    ...
    if stdout_mod != _identity and stdout_file is None:
        _print_warning("stdout_mod has no effect if stdout_file is not "
                       "specified")
    if stdout_res is not None and stdout_file is None:
        _print_warning("stdout_res has no effect if stdout_file is not "
                       "specified")
    if header_string is not None and stdout_file is None:
        _print_warning("header_string has no effect if stdout_file is not "
                       "specified")
    if header_command is not None and stdout_file is None:
        _print_warning("header_command has no effect if stdout_file is not "
                       "specified")
    if header_string is not None and header_command is not None:
        _print_warning("header_string and header_command specified"
                       " - Which one should I use?")
    if header_mod != _identity \
       and header_string is None and header_command is None:
        _print_warning("header_mod has no effect if not one of "
                       "header_string or header_command are specified")

    # generate the set of arguments
    arguments_descr = {k: v if isinstance(v, list) else [v]
                       for k, v in arguments_descr.items()}
    arguments_set = [dict(zip(arguments_descr.keys(), vals)) for vals in
                     itertools.product(*arguments_descr.values())]
...
def _is_skipped(run):
    ...
    '''
    return((run.creates_file is not None and os.path.isfile(run.creates_file))
           or
...
def run():
    ...
    for name, runs in _state.runs_by_name.items():
        ...
        try:
            for _ in tqdm.tqdm(pool.uimap(_run_run, runs),
...
def _run_run(run):
    ...
    # check return codes
    if res.returncode not in run.allowed_return_codes and\
       run.allowed_return_codes != []:
        _print_warning("unexpected return code for command: " +
                       res.args + "\n" + res.stderr.strip() + "")
        return
    ...
    if not os.path.isfile(filename) and (run.header_command is not None or
                                         run.header_string is not None):
    ...
    mod = run.stdout_mod
    output = mod(stdout) if len(signature(mod).parameters) == 1 \
        else mod(stdout, res)
...
def _print_runs():
    ...
    for group, names in _state.groups.items():
        ...
        for name in names:
            ...
            print(_color_selected(
                format_str.format(prefix + name,
